...
export const WindowManagerProvider = ({ children }: { children: ReactNode }) => {
	...
	const openWindow = (app: AppDefinition, appId: string, params: Record<string, WindowParamType> = {}) => {
		...
		const id = nanoid()
		params["windowId"] = id
		const newWindow: WindowInstance = { title: app.name, isPinned: app.isPinned, icon: app.icon, component: app.component, appId, params, id, isFocused: true, size, position: {x: Math.max(0, 50 - size.width / 2), y: Math.max(0, 50 - size.height / 2)}, isMaximized: false, sizeLocked: false }
	...
	return (
		<WindowManagerContext.Provider value={{ windows, openWindow, closeWindow, minimizeWindow, maximizeWindow, focusWindow, moveWindow, resizeWindow, renameWindow }}>
			{children}
